...
import { MigracionSyncHandler } from './migracion-sync.handler';
...
describe('MigracionSyncHandler', () => {
  function crearHandler(opciones: { habitanteExistente?: { id: number } | null } = {}) {
    ...
    const handler = new MigracionSyncHandler(migracionService as never, habitanteService as never, authService as never, registry as never);
    return { handler, habitanteService, migracionService, registry };
  }

  it('se registra en el SyncHandlerRegistry bajo el dominio "movimientos-migratorios"', () => {
    const { handler, registry } = crearHandler();
    handler.onModuleInit();
    expect(registry.registrar).toHaveBeenCalledWith('movimientos-migratorios', handler);
  });

  it('resuelve habitanteUuid -> habitanteId antes de crear el evento', async () => {
    const { handler, habitanteService, migracionService } = crearHandler({ habitanteExistente: { id: 20 } });
    ...
    expect(migracionService.crearParaHabitante).toHaveBeenCalledWith(20, expect.objectContaining({ periodoCensalId: 1 }), expect.anything());
    expect(resultados).toEqual([expect.objectContaining({ uuid: 'movimiento-uuid-1', estado: 'aplicado' })]);
  });

  it('marca error (para reintento) si el habitante todavía no está sincronizado', async () => {
    const { handler, migracionService } = crearHandler({ habitanteExistente: null });

    const resultados = await handler.aplicarLote([crearOperacion()], 1);

    expect(migracionService.crearParaHabitante).not.toHaveBeenCalled();
